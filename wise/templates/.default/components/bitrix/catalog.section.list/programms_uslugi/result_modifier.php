...
if(CModule::IncludeModule("iblock")) {
    ...
    $arSelectFields = array('ID', 'CODE','NAME', 'DETAIL_PAGE_URL', 'PROPERTY_NAME_LIST', 'PROPERTY_LIST_PICTURE', 'PROPERTY_LIST_TEXT', 'PROPERTY_PRICE_LIST', 'PROPERTY_headlink');
    $rsElements = CIBlockElement::GetList($arOrder, $arFilter, FALSE, FALSE, $arSelectFields);
    while ($arElement = $rsElements->GetNext()) {
...
// если включен параметр для задания сортировки по ID'шникам
if($arParams['SORT_BY_FILTER_ID']) {
	// сортируем массив по опеределенной в фильтре позициям в $arParams['FILTER_VALUE']['ID']
	$arSortResult = range(0, count($arResult['SECTIONS']) - 1); // массив отсортированных
	
	foreach ($arResult['SECTIONS'] as $item) {
		// может быть в наборе собственные решения WiseAdvice, они идут символьными кодами в фильтре
		$pos = array_search($item['CODE'], $arParams['FILTER_VALUE']['ID']);
		if (!empty($pos)) {
			$arSortResult[ intval($pos) ] = $item;
			continue;
		}
		
		if (isset($item['ID'])) {
			$pos = array_search($item['ID'], $arParams['FILTER_VALUE']['ID']);
			$arSortResult[ intval($pos) ] = $item;
		}
	}
	
	$arResult['SECTIONS'] = $arSortResult;
}
