...
<?
// код, блокирующий вывод блока с видео в футере. временное решение, т.к.
// @todo есть сомнения, что задача по блокировки вывода блока с видео будет актуальна.
if ($arResult["VARIABLES"]["SECTION_CODE"] == 'dokumentooborot-8') {
	global $isNoBlogBlock;
	$isNoBlogBlock = true;
}
if ($arResult["VARIABLES"]["SECTION_CODE"] == 'uh-8') {
	$vnedrenieLink = '/uslugi-1s/vnedrenie/1s-upravlenie-holdingom/';
}
?>
<? if(in_array($arResult['VARIABLES']['SECTION_CODE'], $arParams['SPLITTED_CONFIGURATIONS'])): ?>
	<?

	switch ($arResult['VARIABLES']['SECTION_CODE']) {
		case 'zup-8':
			$arResult['VARIABLES']['ELEMENT_CODE'] = 'zup-base';
			$arResult['CUT_ELEMENT_CODE_PART'] = 'zup-';
			break;
		case '1s-crm':
			$arResult['VARIABLES']['ELEMENT_CODE'] = 'crm-base';
			$arResult['CUT_ELEMENT_CODE_PART'] = ['crm-', 'srm-'];
			break;
		case '1s-roznica-8':
			$arResult['VARIABLES']['ELEMENT_CODE'] = 'roznica-base';
			$arResult['CUT_ELEMENT_CODE_PART'] = 'roznica-';
			break;
		case 'dokumentooborot-8':
			$arResult['VARIABLES']['ELEMENT_CODE'] = 'doc-korp';
			$arResult['CUT_ELEMENT_CODE_PART'] = 'docs-';
			break;
		case 'upravlenie-nebolshoi-firmoi-8':
			$arResult['VARIABLES']['ELEMENT_CODE'] = 'unf-base';
			$arResult['CUT_ELEMENT_CODE_PART'] = 'unf-';
			break;
		case 'upravlenie-torgovlei-8':
			$arResult['VARIABLES']['ELEMENT_CODE'] = 'ut-base';
			$arResult['CUT_ELEMENT_CODE_PART'] = 'ut-';
			break;
		case 'buhgalteriya-8':
			$arResult['VARIABLES']['ELEMENT_CODE'] = 'base';
			$arResult['CUT_ELEMENT_CODE_PART'] = '';
			break;
	}
	$elementId = $APPLICATION->IncludeComponent(
		...
		array(
			...
			'CUT_ELEMENT_CODE_PART' => $arResult['CUT_ELEMENT_CODE_PART'],
			...
			"HIDE_PREORDER_BLOCK" => $arParams['HIDE_PREORDER_BLOCK']
		),
		$component
	);
...
<?$intSectionID = $APPLICATION->IncludeComponent(
	...
	array(
		...
		"SHOW_TITLE" => $arParams['SHOW_TITLE'],
		"TITLE" => $arParams['TITLE'],
		"AUTOMATIZATION_LAZY_LOAD" => $arParams['AUTOMATIZATION_LAZY_LOAD'],
		"HIDE_PREORDER_BLOCK" => $arParams['HIDE_PREORDER_BLOCK'],
		'VNEDRENIE_LINK' => $vnedrenieLink
	),
	$component
);?>
<? endif; ?>
