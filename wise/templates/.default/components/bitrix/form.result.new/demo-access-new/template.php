...
<div class="<?= $arParams['FORM_CONTAINER_CLASS'] ?> js-typical-form " id="<?= $arParams['FORM_CONTAINER_ID'] ?>">
    ...
    <? if ($arResult["isFormNote"] != "Y") : ?>
        ...
        <input type="hidden" data-fieldtype="formName" value="DemoAccessNew"/>
        ...
        <? foreach ($arResult["VISIBLE_QUESTIONS"] as $kkey => $arQuestion): ?>
            ...
            <? elseif ($arQuestion["CODE"] == "FORM_TARGET"): ?>
                <div class="contact-modal__field _lined">
                    ...
                    <div class="contact-modal__item">
                        <div class="contact-modal__inputs _left _full-width-select">
                            <div class="contact-modal__inputs-select contact-modal__repeat-item ">
                                <select data-fieldtype="tocomment" data-fieldlabel="<?= $arQuestion["CAPTION"] ?>"
                                        class="select2" name="form_<?= $arQuestion["STRUCTURE"][0]["FIELD_TYPE"] ?>_<?= $arQuestion["STRUCTURE"][0]["ID"] ?>" >
                                    <? foreach ($arResult["TARGET"] as $programm): ?>
                                        <option value="<?= $programm ?>"><?= $programm ?></option>
                                    <? endforeach ?>
                                </select>
                            </div>
        ...
        <? endforeach; ?>
        <?//= "</div>" ?>
        <div class="contact-modal__footer _full">
            ...
            <div class="contact-modal__footer-remark">
                Демо-доступ предоставляется аффилированной компанией группы «WiseAdvice» – ООО «Аутсорсинг для бизнеса»
                (ИНН 7721643480) по договору «1С:Аренда ПО» № 010415-49225-45 от «01» апреля 2015 г.
            </div>
        </div>
