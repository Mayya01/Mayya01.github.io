...
foreach ($arResult["QUESTIONS"] as $kkey => $field) {
    ...
    } elseif ($kkey == "FORM_CALLBACK") {
        $arResult["FORM_CALLBACK"] = $field;
...
if(isset($arParams["SUBMIT_TEXT"]) && strlen($arParams["SUBMIT_TEXT"]) > 0) {
	$arResult['SubmitText'] = $arParams["SUBMIT_TEXT"];
} else {
	$arResult['SubmitText'] = "Отправить заявку";
}

// заменяем путь для передачи данных веб-формы
$arResult["FORM_HEADER"] = preg_replace("'action=\".*?\"'si", 'action="/ajax/forms/programms-callback/"',
