...
foreach ($arResult["QUESTIONS"] as $kkey => $field) {
	...
	}  elseif ($kkey == "FORM_EMAIL") {
        $arResult["FORM_EMAIL"] = $field;
    }  else {
...
if(isset($arParams["SUBMIT_TEXT"]) && strlen($arParams["SUBMIT_TEXT"]) > 0) {
	...
	$arResult['SubmitText'] = "Получить";
}

// заменяем путь для передачи данных веб-формы
$arResult["FORM_HEADER"] = preg_replace("'action=\".*?\"'si", 'action="/ajax/forms/get-examples-edo/"',
