/*$.fn.initWebFormContacts = function(){
	var form = $(this);
	...
	function inputChecker($input) {
		...
		// @todo интегрировать проверку полей для формы ARTICLES_SUBSCRIBE

		var $field = $input.closest('.contact-modal__field');
		if ($field.hasClass('_error, _none')) return false;
		if ($input.data('necessary') == undefined && $input.val() == '') return true;
		...
		switch (type) {
			case 'checkbox':
				console.log($input);
				if (!$input.prop('checked') && $input.data('necessary') != undefined) {
			...
			default:
				if ($input.data("mask") == "email") {
					var regExp = /^([A-Za-z0-9_-]+\.)*[A-Za-z0-9_-]+@[A-Za-z0-9_-]+(\.[A-Za-z0-9_-]+)*\.[A-Za-z]{2,6}$/g;
					var result = regExp.test($input.val().trim());
					if (result) {
						return true;
					}
					$field.addClass('_none');
    ...
    $(this).ajaxForm({
		...
		beforeSubmit: function(){
			if (form.find('input[name="form_text_007"]').val() != ''){
				// если скрытое текстовое поле заполнено - значит заполнил бот, блокируем обработку формы
				return false;
			}

			// проверка на количество заполнений формы, если >3, блокируем отправку
			var formCounter = getCookie('wa_form_' + form.attr('name') + '_send_counter');
			if (formCounter >= 2) {
				$.fancybox("#no-more-forms");
				$('#no-more-forms .c-subscription-form').show();
				return false;
			}
		...
		success: function(data){
			...
			if(data.status == 'error'){
				form.closest(".js-form-wrapper").removeClass("is-loading");
				...
				if (typeof SubmitSender2Mail2CT == "function") {
                    ...
                    var commentStr = "";
                    var formName = "Заполненная форма: ";
                    form.find('[data-fieldtype]').each(function () {
                        var field = $(this);
                        if (field.data("fieldtype") == "username") {
                            snd_obj.name = field.val();
                        }
                        else if (field.data("fieldtype") == "phone") {
                            snd_obj.phone = field.val();
                        }
                        else if (field.data("fieldtype") == "email") {
                            snd_obj.email = field.val();
                        }
                        else if (field.data("fieldtype") == "direction") {
                            snd_obj.direction = field.val();
                        }
                        else if (field.data("fieldtype") == "tocomment") {
                            if (field.attr("type") == "checkbox") {
                                if (field.prop("checked")) {
                                    if (typeof field.data("fieldlabel") != undefined)
                                        commentStr += " ||| " + field.data("fieldlabel") + " ||| ";
                                }
                            } else if (field.attr("type") == "radio") {
                                if (field.prop("checked")) {
                                    commentStr += " ||| " + field.data("fieldlabel") + ":" + field.val() + " ||| ";
                                }
                            } else {
                        ...
                        else if (field.data("fieldtype") == "formName") {
                            formName += field.val() + "(" + window.location.href + ")";
                        }
                    });

                    snd_obj.comment = formName + commentStr;

                    var zatoJson = SubmitSender2Mail2CT(snd_obj);

                    var Context = { ZATO: zatoJson };

                    $.post("/local/ajax/io/zato.php", Context, function (data) {
