...
var FileIconInput = {
	...
	changeValue: function (evt) {
		...
		this.downoladField.html('Загружено (' + '<span>1</span>' + ')').on('click', function (evt) {
			if (self.toolTip.hasClass('hidden')) {
				self.toolTip
					.removeClass('hidden')
				...
				self.toolTip.find('.js-form-card-block-file__tooltip-wrapper').text('');
				self.removeFile4List(evt);
			}
	...
	removeFile4List: function (evt, tooltip) {
		...
		this.downoladField.removeClass(' bordered').html(this.downoladFieldText).off('click');
