$(document).ready(function() {
	...
	$('.text-container-rd .container-rd').each(function(){
		if($(this).children().length===0) {
			$(this).remove();
		}
	});

	/** Получаю историю переходов по страницам */
	let serfHistory = JSON.parse(sessionStorage.getItem('serfHistory'));
	if (serfHistory === null) {
		serfHistory = [];
	}
	/** Добавляю h1, title и url тукущей страницы в  serfHistory*/
	if (document.getElementsByTagName('h1').length) {
		serfHistory.unshift({
			H1: document.getElementsByTagName('h1').item(0).innerHTML,
			title: window.pageTitle = document.title,
			path: window.location
		});
	};

	/** Ограничиваю длину serfHistory 2 элементами */
	serfHistory.length = 2;

	/** Сохраняю историю переходов по страницам */
	sessionStorage.setItem('serfHistory', JSON.stringify(serfHistory));


	/** заполняю поле значением h1 страницы с которой перешел на текущую */
	let entryH1 = $('.js-entry-h1');
	if (entryH1.length > 0) {
		entryH1.val(serfHistory[1].H1);
	}
...
App.Control.install({
	...
	initialize: function () {
		...
		if (this.$el.hasClass('js-dotted-nav-slider--rd')) {
			this.controls = true;
		} else {
			this.controls = false;
		}
...
var EqualHeight = {
	...
	setEqualHeight: function () {
		...
		if ($(window).width() < 768 && this.block.hasClass('js-equal-height__block--fix-adaptive')) {
			this.block.css('height', 'auto');
		}
	}
};
...
var ExpertsBlock = {
	...
	initialize: function () {
		...
		this.contentHeight = this.expertSections.find('.expert-rd__expert-info').outerHeight();
		this.askBtn = this.$('.js-experts-block__ask-btn');
		this.askBtn.css('top', this.contentHeight);
		var self = this;

		$(window).bind('resize', function () {
			 self.expertSections.find('.expert-rd__expert-info').each(function (item) {
			 	if($(this).outerHeight() > 0 ) {
					self.askBtn.css('top', $(this).outerHeight());
				}
			 });
		});
	},
	...
	activeExperts: function (evt) {
		...
		var targetHeight = $('#' + this.targetId).find('.expert-rd__expert-info').outerHeight();
		this.askBtn.css('top', targetHeight);
	}
};
...
App.Control.install({
   ...
   self.disableModals();
		...
		let formType = this.$el.find('input[name="form"]').val();
...
var Header = {
	...
	name: 'Header',
		...
		$(window).bind('resize', function () {
			self.blockHeight = self.$el.outerHeight();
			self.blockOffset = self.$el.offset().top;
		});
...
var LogosSliderRd = {
	...
	renderMode: function () {
		...
		if ($(window).width() < self.breakpoint) {
			self.destroySliderLg();
			self.initSliderMd();
		} else {
			self.destroySliderMd();
			self.initSliderLg();
		}
	},
	...
	initSliderLg: function () {
		var self = this;
		self.slidesWrapperLg.append(self.slidesLg);
		if (!this.sliderLg) {
	...
	destroySliderMd: function () {
		...
		self.slidesWrapperMd.empty();
	},

	initSliderMd: function () {
		var self = this;
		self.slidesWrapperMd.append(self.slidesMdClone);
		if (!this.sliderMd) {
...
var MainOfficeMapRd = {
	...
	initialize: function () {
		if (!_.isUndefined(window.ymaps)) {
			...
			function init() {
				myMap = new ymaps.Map("main-office-rd", {
					center: [55.718324068999664, 37.79198949999998],
					zoom: 15,
					controls: []
				});
...
var MobileHidden = {
		...
		events: {
				'click .js-mobile-hidden__link' : 'contentToggle'
		},

		contentToggle: function(evt) {
				evt.preventDefault();
...
var PriceCard = {
    ...
    toggleActive: function(e) {
        this.card.removeClass(this.cls);
        $(e.currentTarget).addClass(this.cls);
    },
    toggleInActive: function(e) {
        $(e.currentTarget).removeClass(this.cls);
        this.default.addClass(this.cls);
    }
};
...
var PublicationsSlider = {
	...
	initialize: function () {
		...
		const controls = this.$el.parents('.js-slick-slider-wrapper');
		let showControl = false;

		if (controls.find('.slick-slider-element').length > 1) {
			showControl = true;
		}
		...
		this.$el.slick({
			...
			appendArrows: $('.js-slick-slider-wrapper__controls', controls),
			appendDots: $('.js-slick-slider-wrapper__controls', controls),
			responsive: [
				{
					breakpoint: 991,
					settings: {
...
App.Control.install({
	...
	renderMode: function () {
		var self = this;
		if ($(window).width() <= 767 || this.sliderLenght > 3) {
			self.initSlider();
		} else {
			self.destroySlider();
		}
	...
	initSlider: function () {
		var self = this;
		if (!this.slider) {
			this.slider = this.$el.bxSlider(self.settingsHandler());
		}
	},
	enableGallery:function(){
		var self = this;
		this.$el.reloadSlider(self.settingsHandler());
	},
	destroySlider: function () {
		if (this.slider) {
			this.slider.destroySlider();
...
var SliderPriceCards = {
	...
	initSlider: function() {
		var self = this;
		if(this.$el.is('[data-adaptive-height]') && $(window).width() < 768) {
			this.adaptive = true;
		} else {
			this.adaptive = false;
		}
...
var StickyMediaLogo = {
	...
	stickyOnScroll: function() {
		if($(window).scrollTop() >= this.pushPoint && $(window).scrollTop() < this.stopPoint && $(window).width() >= 768) {
...
var TabsControl = {
	...
	equalWidth: function () {
		...
		this.tab.each(function(){
			var tabWidth = parseFloat($(this).outerWidth())
			if (tabWidth > maxWidth) {
				maxWidth = tabWidth;
			}
		});
		this.tab.css('min-width', maxWidth);
	}
};
...
var VerticalTabs = {
	...
	initialize: function () {
		...
		if(window.location.hash && this.$el.find('.js-vertical-tabs__tab[data-id="' + window.location.hash.replace('#', '') + '"]').length) {
			this.tabContent.removeClass('is-active');
			$(window.location.hash).addClass('is-active');
			this.tab.removeClass('is-active');
			this.$el.find('.js-vertical-tabs__tab[data-id="' + window.location.hash.replace('#', '') + '"]').addClass('is-active');
		}
...
var MainNavView = {
	...
	fixedNav: function () {
		if ($(window).width() > 900) {
			...
			if ($(window).scrollTop() < this.mainNavOffsetTop) {
				this.$el.removeClass('main-nav--fixed');
			} else if ($(window).scrollTop() > this.mainNavOffsetTop && !this.$el.hasClass('js-main-nav--no-fixed')) {
				this.$el.addClass('main-nav--fixed');
			} else if ($(window).scrollTop() > this.mainNavOffsetTop && this.$el.hasClass('js-main-nav--no-fixed')) {
				this.$el.removeClass('main-nav--fixed');
			}
...
var FileIconInput = {
	...
	initialize: function () {
		...
		this.downoladFieldText = this.$('.js-form-card-block-file__download-field').text();
		this.textInput = this.$('input[type="text"]');

		var self = this;
		this.toolTip = self.createTooltip();
	...
	changeValue: function (evt) {
		...
		if ($(window).width() > self.breakpoint) {
			self.getTooltipLg(fileName);
		} else {
			self.getTooltipMd(fileName);
		}

	},
	getTooltipLg: function (fileName) {
		var self = this;
		this.fileInput.appendTo(this.$el);
		
		this.downoladField.html('<span class="bordered">Загружено (' + '<span>1</span>' + ') </span>');

		this.downoladFieldLabel.hover(function () {
			self.toolTip
				.removeClass('hidden')
	...
	getTooltipMd:function(fileName){
		var self = this;
	...
	createTooltip: function () {
		...
		var tooltip = $(document.createElement('div'))
			.addClass('form-tooltip js-form-card-block-file__tooltip');

		var tooltipWrapper = $(document.createElement('span'))
			.addClass('form-tooltip__wrapper js-form-card-block-file__tooltip-wrapper');

		var tooltipClose = $(document.createElement('span'))
			.addClass('form-tooltip__close js-form-card-block-file__tooltip-close');

		var tooltipArrow = $(document.createElement('span'))
			.addClass('form-tooltip__arrow');


		tooltipWrapper.appendTo(tooltip);

		tooltipClose.appendTo(tooltip);

		tooltipArrow.appendTo(tooltip);

		tooltip.appendTo(this.downoladFieldLabel);

		tooltip.addClass('hidden');
		return tooltip;

	},
	removeFile4List: function (evt, tooltip) {
		evt.preventDefault();
