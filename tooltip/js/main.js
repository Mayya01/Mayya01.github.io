...
var FileIconInput = {
	...
	initialize: function () {
		...
		this.downoladField = this.$('.js-form-card-block-file__downolad-field');
		this.downoladFieldText = this.$('.js-form-card-block-file__downolad-field').text();
	...
	changeValue: function (evt) {
		...
		this.downoladField.html('Загружено (' + '<span>1</span>' + ')').addClass('bordered').one('click', function (evt) {
			/*evt.preventDefault();
	...
	removeFile4List: function (tooltip) {
		this.toolTip.hide();
		this.downoladField.removeClass('bordered').html(this.downoladFieldText);
